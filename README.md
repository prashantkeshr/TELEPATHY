...
- **Icons:** `public/favicon.svg` is the single source for the mark. `node scripts/generate-icons.mjs` regenerates every PNG, the maskable icon, a real multi-size `favicon.ico` (16/32/48), and `og-image.png`, and fails if the mark is off-centre in its tile. `safari-pinned-tab.svg` is a separate monochrome silhouette (Safari ignores colour there).
